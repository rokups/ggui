...
void Application::RenderChanges()
{
    ...
    // Keyboard file navigation; Shift extends the selection and Ctrl+A
    // selects every listed file.
    const ImGuiIO& io = ImGui::GetIO();
    const bool list_focused = !io.WantTextInput && ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows);
    const bool keyboard_navigation = list_focused && !io.KeyCtrl && !io.KeyAlt && !io.KeySuper;
    if (list_focused && io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(ImGuiKey_A, false))
        SelectAllChangedFiles();
    ...
    if (navigate_up || navigate_down)
        NavigateChangedFile(navigate_down ? 1 : -1, io.KeyShift);
    const bool selection_navigated = previous_file != _selected_file;

    // Multi-selection context menu. Paths are collected only when an action
    // runs, so an open menu does not allocate every frame.
    const auto selected_paths = [this]() {
    ...
    const auto selected_entries = [this]() {
    ...
    const auto render_selection_menu = [&](std::size_t count) {
        ImGui::TextDisabled("%zu files selected", count);
        ImGui::Separator();
        const std::string copy_label = IconLabel(ICON_MS_CONTENT_COPY, "Copy");
        if (ImGui::BeginMenu(copy_label.c_str()))
        {
            const auto copy = [&](auto&& format) {
                std::string text;
            ...
            if (ActionMenuItem(ICON_MS_CONTENT_COPY, "Names"))
                copy([](const std::string& path) { return std::filesystem::path(path).filename().string(); });
            if (ActionMenuItem(ICON_MS_CONTENT_COPY, "Relative paths"))
                copy([](const std::string& path) { return path; });
            if (ActionMenuItem(ICON_MS_CONTENT_COPY, "Absolute paths"))
                copy([this](const std::string& path) {
                    const std::optional<std::filesystem::path> absolute = WorkingCopyPath(_snapshot->root, path);
                    return absolute.has_value() ? absolute->string() : path;
                });
            ImGui::EndMenu();
        }
        ImGui::Separator();
        ...
        for (const StatusEntry& file : _diff.files)
            any_conflicted = any_conflicted || (file.conflicted && IsChangedFileSelected(file.path));
        ImGui::BeginDisabled(actions_locked || comparison_active || _diff_loading || CurrentCommit(*_snapshot).empty()
            || (working_tree_diff && any_conflicted));
        ...
        if (ActionMenuItem(ICON_MS_ARROW_UPWARD, active_commit_diff ? "Move to Working tree" : "Move to child"))
            QueueCommands({MoveFiles{_diff.revision, move_child, selected_paths()}}, {_diff.revision, move_child},
        ...
        if (ActionMenuItem(ICON_MS_ARROW_DOWNWARD, working_tree_diff ? "Move to active commit" : "Move to parent"))
            QueueCommands({MoveFiles{_diff.revision, move_parent, selected_paths()}}, {_diff.revision, move_parent},
                "Moving these files will rewrite a locked source or destination commit.");
        ImGui::EndDisabled();
        if (_selected_revision == CurrentCommit(*_snapshot) || IsWorkingTreeRevision(_selected_revision))
        {
            ...
            if (ActionMenuItem(ICON_MS_ADD, "Track"))
                QueueCommands({TrackPaths{selected_paths()}}, {"@"},
                    "Tracking these files will rewrite the locked active commit.");
            if (ActionMenuItem(ICON_MS_DELETE, "Untrack"))
                QueueCommands({UntrackPaths{selected_paths()}}, {"@"},
                    "Untracking these files will rewrite the locked active commit.");
            ImGui::Separator();
            // Deleting several files at once is confirmed first; files that
            // are already gone from disk are skipped.
            if (ActionMenuItem(ICON_MS_DELETE, "Delete files"))
            {
                std::vector<StatusEntry> files = selected_entries();
                std::erase_if(files, [this](const StatusEntry& file) {
    ...
    for (const StatusEntry& file : _diff.files)
    {
        ...
        const bool selected = ImGui::Selectable(item_id.c_str(), IsChangedFileSelected(file.path), 0,
            ImVec2(0.0f, 26.0f));
        ...
        if (selected)
            ClickChangedFile(file.path, io.KeyCtrl, io.KeyShift);
        ...
        {
            // Dragging a selected file carries the whole selection.
            std::string payload = _diff.revision;
            payload.push_back('\0');
            std::size_t count = 0;
            for (const StatusEntry& dragged : _diff.files)
            {
                if (dragged.path != file.path
                    && (!IsChangedFileSelected(file.path) || !IsChangedFileSelected(dragged.path)))
                    continue;
                payload += dragged.path;
                payload.push_back('\0');
                ++count;
            }
            ...
            if (count == 1)
                ...
                ImGui::Text("Move %zu files", count);
        ...
        {
            // Right-clicking outside the selection selects that file alone.
            if (!IsChangedFileSelected(file.path))
                SelectFile(file.path);
            std::size_t selected_count = 0;
            for (const StatusEntry& candidate : _diff.files)
                selected_count += IsChangedFileSelected(candidate.path) ? 1 : 0;
            if (selected_count > 1)
                render_selection_menu(selected_count);
            else
            {
                const std::optional<std::filesystem::path> absolute = WorkingCopyPath(_snapshot->root, file.path);
